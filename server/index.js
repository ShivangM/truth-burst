...
const io = new Server(server, {
  cors: {
    origin: [
      'http://localhost:3000',
      'https://www.truthburst.live',
      'https://truth-burst.netlify.app',
    ],
    methods: ['GET', 'POST'],
    credentials: true,
  },
...
io.on('connection', (socket) => {
  ...
  socket.on('join', async ({ name, room }, callback) => {
    ...
    if (!existingRoom)
      return callback({ error: 'Please join with valid room code' });
  ...
  socket.on('generateQuestion', async (room, callback) => {
    let error = false;
    let { prevQuestions, prevUsers } = await Round.findOne({
      room: room,
    }).exec();
    ...
    const users = await User.find({
      $and: [{ room: room }, { _id: { $nin: prevUsers } }],
    });
    ...
    randomQuestion = randomQuestion.question.replace(
      NAME_REPLACER,
      randomUser.name
    );
    ...
    await Round.findOneAndUpdate(
      { room: room },
      { currQuestion: randomQuestion, prevQuestions, prevUsers }
    );
    ...
    if (user) {
      ...
      try {
        await Answer.findOneAndDelete({
          $and: [{ name: user.name }, { room: user.room }],
        }).exec();
        checkAnswer(user.room);
